...
const joinQueue = async (req, res) => {
  try {
    ...
    const existingToken = await Token.findOne({ userId, serviceCenterId, status: 'waiting' })
    if (existingToken) return res.status(400).json({ message: 'You are already in this queue' })
    ...
    const waitingCount = await Token.countDocuments({ serviceCenterId, counterId, status: 'waiting' })
...
const getMyToken = async (req, res) => {
  try {
    const token = await Token.findOne({ userId: req.user.userId, status: 'waiting' })
      .populate('serviceCenterId', 'name location')
    if (!token) return res.status(404).json({ message: 'No active token found' })
...
const cancelToken = async (req, res) => {
  try {
    const token = await Token.findOne({ _id: req.params.id, userId: req.user.userId })
    if (!token) return res.status(404).json({ message: 'Token not found' })

    token.status = 'cancelled'
    await token.save()

    await ServiceCenter.findByIdAndUpdate(token.serviceCenterId, { $inc: { totalWaiting: -1 } })
...
export { getAllServiceCenters, getServiceCenterById, joinQueue, getMyToken, cancelToken }