...
function Home() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center">
      <Navbar />
      <p className="text-gray-500 dark:text-gray-400">Loading service centers...</p>
    </div>
  )
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors duration-300">
      ...
      <div className="bg-linear-to-r from-indigo-600 to-indigo-400 pt-28 pb-20 px-6">
        <div className="max-w-5xl mx-auto text-center">
          <p className="text-indigo-200 text-sm font-medium mb-1">Welcome to QueueLess</p>
          <h1 className="text-3xl font-bold text-white mb-2">Find a Service Center</h1>
          <p className="text-indigo-100 text-sm mb-8">Join a digital queue and skip the wait</p>
          ...
          <div className="flex items-center gap-4 mt-6 flex-wrap justify-center">
            ...
            <div className="flex items-center gap-2 bg-white/10 text-white text-xs font-medium px-4 py-2 rounded-full">
              <span>✅</span>
              <span>{getTotalActiveCounters(serviceCenters)} Active Counters</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 pt-8 pb-12">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 dark:text-gray-400 font-medium">No centers found for "{search}"</p>
            <p className="text-sm text-gray-400 mt-1">Try searching by hospital, bank or government</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((center) => {
              const config = categoryConfig[center.category] || categoryConfig['hospital']
              return (
                <div
                  key={center._id}
                ...
                >
                  <div className="p-5 pb-4">
                    ...
                    <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-0.5">{center.name}</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{center.type}</p>
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">📍 {center.location}</p>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-0 border-t border-white/60 dark:border-white/5">
                    <div className="p-3 text-center border-r border-white/60 dark:border-white/5">
                      <p className={`text-base font-bold ${config.stat}`}>{center.totalWaiting}</p>
                      <p className="text-xs text-gray-400 mt-0.5">Waiting</p>
                    </div>
                    <div className="p-3 text-center border-r border-white/60 dark:border-white/5">
                      <p className={`text-base font-bold ${config.stat}`}>{getEstimatedWait(center.totalWaiting)}</p>
                      <p className="text-xs text-gray-400 mt-0.5">Est. Wait</p>
                    </div>
                    <div className="p-3 text-center">
                      <p className={`text-base font-bold ${config.stat}`}>{center.counters.length}</p>
